...
int main(){
    ...
    // не работает, потому что ptr типа FirearmsClass, в таком классе нет реализации setAutomaticMode()
    // shoot() сработал, потому что он определён в базовом классе, и тем более в дочернем и он переопределяется
