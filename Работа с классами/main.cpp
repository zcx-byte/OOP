...
int main(){
    ...
    std::cout << User1.get_balance() << std::endl;
    std::cout << User2.get_balance() << std::endl;
    std::cout << User3.get_balance() << std::endl;
    ...
    BankUserClass file_user_1("Файловый1", "F_s_1", 999.9);
    ...
    {
        std::ifstream Input_filename(input_filename);
        std::string line;
        while (std::getline(Input_filename, line)) {
    ...
    // делаем count указателей на BankUserClass*
    ...
    for (int i = 0; i < count; i++) {
        ...
        users_from_file[i]->load_from_file(input_filename);      // перезаписываем
    }
